    ...
    <section class="flex justify-center max-w-screen-lg m-auto">
        ...
        <div id="indicators-carousel" class="relative w-full" data-carousel="static">
            ...
            <div class="relative min-h-[280px] overflow-hidden lg:rounded-lg rounded-none md:h-[440px]">
                <!-- Item 1 -->
                @foreach ($vehicle->images()->sortByDesc('is_featured') as $image)
                    <img class="hidden w-full h-full object-cover duration-700 ease-in-out"
                        ...
                        src="{{ $image->file_path }}" alt="{{ $image->name ?? 'Vehicle Image ' . $loop->index }}">
                @endforeach
            </div>
            <!-- Slider indicators -->
            <div class="absolute z-30 flex -translate-x-1/2 space-x-3 rtl:space-x-reverse bottom-5 left-1/2">
                @foreach ($vehicle->images()->sortByDesc('is_featured') as $index => $image)
