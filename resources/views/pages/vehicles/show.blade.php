<div class="max-w-screen-xl m-auto">
    ...
    <div class="mx-2 xl:mx-0 mt-2 md:mt-4 xl:mt-6">
        <div class="flex flex-wrap md:flex-nowrap gap-4">
            ...
            <x-div class="w-full md:w-3/5 order-1 my-0">
                <div class="hidden md:block">
                    <h1 class="text-2xl capitalize">{{ $vehicle->name ?? '' }}</h1>
                    <div class="flex items-baseline ">
                        <span class="flex items-center !text-[21px] font-semibold"
                            title="This is not the actual rating for your vehicle">{{ $vehicle->rating ?? 4.9 }}<svg
                        ...
                        <span class="ml-2"
                            title="This is not the actual trips your vehicle has taken">({{ '0 trips' }})</span>
                    </div>
                </div>
            ...
            <x-div class="w-full md:w-2/5 order-first md:order-2 my-0">
                <div class="md:hidden">
                    <h1 class="text-2xl capitalize">{{ $vehicle->name ?? '' }}</h1>
                    <div class="flex items-baseline ">
                        <span class="flex items-center !text-[21px] font-semibold">{{ $vehicle->rating }}
                        ...
                        <span class=" ">({{ '5 trips' }})</span>
                    </div>
                </div>
                <div class="flex flex-nowrap items-baseline gap-2">
                    @if ($vehicle->on_sale)
                        <span class=" text-lg line-through opacity-60">{{ $vehicle->human_price }}</span>
                        <span class="text-2xl font-semibold">{{ $vehicle->human_sale_price }}</span>
                    @else
                        <span class="text-2xl font-semibol">{{ $vehicle->human_price }}</span>
                    @endif
                    / day
                </div>

                <p class="text-xs hover:underline font-light mt-4">The discount is applied if the vehicle is rented for
        ...
        {{-- Description --}}
        {{-- Details --}}
        <x-accordion id="description-accordion">
            <x-accordion-item id="description-accordion-item-1">
                ...
                <x-slot name="content">
                    <p class=" mt-6"> {{ $vehicle->description }}</p>
        ...
        {{-- Features --}}
        <x-accordion id="features-accordion">
            <x-accordion-item id="features-accordion-item-1">
                <x-slot name="title">
                    <h3 class="text-xl">Features</h3>
                </x-slot>

                <x-slot name="content">
                    <div class="columns-2 md:columns-3 gap-4 mt-4 space-y-4">
