...
<select {{-- Load the function if loadJS attribuite is set to true --}}
    @if ($loadJS === 'true' || $loadJS) x-data="{setDefaultOption(){const selectElements=document.querySelectorAll('select[data-default-value]');selectElements.forEach(select=>{const defaultValue=select.getAttribute('data-default-value');Array.from(select.attributes).forEach(attr=>{if(attr.name.startsWith('wire:')){if(!@this.get(`${attr.value}`)){if(defaultValue){@this.set(`${attr.value}`,defaultValue);select.value=defaultValue;}else{let defaultOption=Array.from(select.options).find(option=>option.hasAttribute('selected'));if(defaultOption&&defaultOption.value){@this.set(`${attr.value}`,defaultOption.value);select.value=defaultOption.value;}else{@this.set(`${attr.value}`,select.value);}}}}});});}}" x-init="setDefaultOption()" @endif
