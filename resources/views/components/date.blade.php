...
<div @if ($loadJS) x-data="{ initDatepicker() { document.querySelectorAll('[datepicker]').forEach($datepickerEl => { Array.from($datepickerEl.attributes).forEach(attr => { if (attr.name.startsWith('wire:')) { $datepickerEl.addEventListener('hide', function() { @this.set(`${attr.value}`, $datepickerEl.value); }); }  }); }); } }" @endif x-init="() => { setTimeout(() => { initDatepickers(); initDatepicker(); }, 500); }" class="relative {{ $width }}">
