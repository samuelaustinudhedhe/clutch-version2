...
<!-- Dropdown menu -->
<x-div class="hidden z-50 my-4 w-56 text-base list-none divide-y divide-gray-200 shadow dark:divide-gray-600 !p-0 overflow-hidden" id="dropdown">
    <div class="py-3 px-4">
    ...
    <ul class="py-1 font-light text-gray-500 dark:text-gray-400" aria-labelledby="dropdown">
        <li>
            <a href="{{ route( (getUser()? 'user' : 'admin') . '.profile.show') }}" class="block py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400 dark:hover:text-white">My profile</a>
        </li>
        <li>
            <a href="{{ route( (getUser()? 'user' : 'admin') . '.settings.show') }}" class="block py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400 dark:hover:text-white">Account settings</a>
        </li>
    </ul>
    <ul class="py-1 font-light text-gray-500 dark:text-gray-400" aria-labelledby="dropdown">
        <li>
    ...
    <ul class="py-1 font-light text-gray-500 dark:text-gray-400" aria-labelledby="dropdown">
        <li>
            {{-- Logout --}}
            <form method="POST" action="{{ route( (getAdmin()? 'admin.' : '') . 'logout') }}" x-data>
                @csrf
                
                <a href="{{ route( (getAdmin()? 'admin.' : '') . 'logout') }}" class="block py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" @click.prevent="$root.submit();">
                {{ __('Log Out') }}
                </a>
            </form>
        </li>
