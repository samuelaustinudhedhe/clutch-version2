            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div
                    ...
                    <div class="flex-1 flex items-center space-x-2">
                        <h5>
                            ...
                            <span class="dark:text-white">{{ $trips->count() }}</span>
                        </h5>
                ...
                <div
                    ...
                    <div class="w-full md:w-1/2">
                        <x-search type="text" id="simple-search" wire:model.live='search'
                            placeholder="Search for products" />
                    </div>
                    <div
                        class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <x-button type="button" href="#">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="p-4">Rented For </th>
                                <th scope="col" class="p-4">Status</th>
                                <th scope="col" class="p-4">Vehicle Price</th>
                                <th scope="col" class="p-4">Distance</th>
                                <th scope="col" class="p-4">Total Paid</th>
                                {{-- <th scope="col" class="p-4">Promote</th> --}}
                                <th scope="col" class="p-4">Pick up</th>
                                ...
                                <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <th scope="row"
                                        ...
                                        <a href="{{ route('admin.trips.show', $trip->id) }}"
                                            class="flex items-center mr-3 break-words ">
                                            {{-- Vehicle Image --}}
                                            <x-img src="{{ $trip->vehicle->featured_image_url }}"
                                                alt="{{ $trip->vehicle->featured_image->name ?? $trip->vehicle->name }}"
                                                title="{{ $trip->vehicle->name }}"
                                                class="min-h-12 h-auto max-h-14 min-w-16 w-20 max-w-20 object-cover mr-3 rounded-md" />
                                            #{{ $trip->id }}
                                        </a>
                                    ...
                                    <td class="px-4 py-3  text-gray-900 dark:text-white">
                                        <div class="flex items-center capitalize ">
                                            <div
                                                class="h-4 w-4 rounded-full inline-block mr-2 bg-{{ $trip->status_color }}-500">
                                            </div>
                                            {{ $trip->status }}
                                        </div>
                                    </td>
                                    ...
                                    {{-- Price --}}
                                    <td class="px-4 py-3">
                                        {{ humanizePrice($trip->vehicle_price) }}
                                    </td>

                                    <td class="px-4 py-3 capitalize">
                                        {{ $trip->max_allowed_distance }}
                                    </td>

                                    {{-- Total Paid price --}}
                                    <td class="px-4 py-3  text-gray-900 dark:text-white">
                                        {{ humanizePrice($trip->total_price) }}
                                    </td>

                                    {{-- Pick up on --}}
                                    <td class="px-4 py-2">
                                        {{ $trip->start_date->format('d M Y, h:i A') }}
                                    </td>

                                    {{-- Actions --}}
                                    <td
                                        class="px-4 py-3 font-medium text-right text-gray-900 whitespace-nowrap dark:text-white">
                                        <button id="dropdown-button-{{ $trip->vehicle->id }}" type="button"
                                            data-dropdown-toggle="dropdown-{{ $trip->vehicle->id }}"
                                        ...
                                        <div id="dropdown-{{ $trip->vehicle->id }}"
                                            class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                            <div class="block px-4 py-2 text-sm text-gray-400">
                                                {{ __('Action Menu') }}
                                            </div>
                                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                aria-labelledby="dropdown-button-{{ $trip->vehicle->id }}">
                                                <li>
                                                    <a data-drawer-target="drawer-update-product"
                                                ...
                                                <li>
                                                    <a data-drawer-target="drawer-read-product-advanced"
                                                ...
                                                <li>
                                                    <a wire:click="deleteVehicles({{ $trip->vehicle->id }})"
