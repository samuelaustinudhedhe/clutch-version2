        ...
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            @foreach ($images['uploaded'] as $index => $image)
                @php
                    ...
                    $fileSize =
                        $fileSizeInBytes > 0 ? number_format($fileSizeInBytes / 1024, 2) . ' KB' : 'Size not available';
                @endphp

                <div class="relative rounded-lg  dark:bg-gray-700 {{ $images['featuredIndex'] === $index ? 'border-4 border-blue-500' : 'border-4 border-transparent' }}"
            ...
            <div class="flex items-center justify-center @if(!$images['uploaded']) col-span-3 @endif ">
                <label for="dropzone-image"
                    ...
                    <x-xinput id="dropzone-image" type="file" wire:model="images.newUploads" multiple class="hidden"  accept=".jpg,.jpeg,.png" />                
                </label>
            </div>
        </div>
