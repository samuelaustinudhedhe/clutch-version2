<div class="flex items-start p-6 py-8 bg-white dark:bg-gray-900 w-full">
    ...
    <div class="container max-w-screen-md mx-auto px-4 md:px-8 md:max-h-[calc(100vh-200px)] md:overflow-auto ">
        ...
        @switch($currentStep)
            ...
            @case(1)
                @include('user.vehicle.wizard.basic-informations')
            @break

            @case(2)
                @include('user.vehicle.wizard.extirior-intirior')
            @break

            @case(3)
                @include('user.vehicle.wizard.engine-transmission')
            @break

            @case(4)
                @include('user.vehicle.wizard.safety-security')
            @break

            @case(5)
                @include('user.vehicle.wizard.faults-modifications')
            @break

            @case(6)
                <!-- Step 6: Vehicle Documents -->
                @include('user.vehicle.wizard.images')
            @break

            @case(7)
                <!-- Step 7: Insurance Documents -->
                @include('user.vehicle.wizard.documents')
            @break

            @case(8)
                <!-- Step 8: Final Confirmation and Submission -->
                @include('user.vehicle.wizard.price')
            @break
            @case(9)
                <!-- Step 8: Final Confirmation and Submission -->
                @include('user.vehicle.wizard.review-submit')
            @break
